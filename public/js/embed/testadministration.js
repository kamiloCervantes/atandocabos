...
testadministration.init = function(){
    ...
    testadministration.profileid = params[3];
    console.log(testadministration.testid);
    console.log(testadministration.profileid);
    testadministration.nextitem();
    $(document).on('keydown', testadministration.debug);
}

testadministration.nextitem = function(){
    var jqxhr = $.get('/api/testadministration/token/'+testadministration.testid+'/pid/'+testadministration.profileid);
    ...
    jqxhr.done(function(data){
        if(data.test_status == 0){
            ...
            $('.opcion-item').remove();
            testadministration.complejidad.html('<p class="pull-right" style="font-style: italic; font-size: 12px;">Complejidad del Ítem: '+data.complejidad+'</p>');
            ...
            $('#test').html(testadministration.endtestviewhelper(data.ability_final));
        }
    });
}
...
testadministration.sendanswer = function(){
    ...
    var profile_token = testadministration.profileid;
...
testadministration.endtestviewhelper = function(ability){
    var html = '';
