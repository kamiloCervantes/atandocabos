...
function cargarGeneral() {
	...
	limpiar(miBoton);
        ...
        $('.next-item').on('click', function(e){
            ...
            $('#index').val(index);
            console.log(index);
            d3.xhr("/servicios/gettolerancia")
            .header("Content-Type", "application/json")  
            .post(
             JSON.stringify({indicador: $('#indicadordata').val(), vista: "todos"}), function(error, data){	
    //             console.log(this);
                 var data = JSON.parse(data.response); 
    //             $.each($(contGrafs), function(i,val){
    //                $(val).addClass('lt-cont-grafs'+i);
                    $('#numsubpreguntas').val(data[0]['pregunta_idPregunta']['subpreguntas'].length);
                    var datos = data[0]['pregunta_idPregunta']['subpreguntas'][index];
                    $('.lt-subtit-ind h3').html(datos[1].titulo);
                    var aux = [];
                    datos = $.each(datos, function(idx, val){
                        if(typeof val == 'object' && val != null){
                            aux.push(val);
                        }                    
                    });
                    datos = aux;
                    var idInd = datos[0].subpregunta;
    //                console.log(datos);
