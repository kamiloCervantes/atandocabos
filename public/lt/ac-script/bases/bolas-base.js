...
function bolasEstructura(losDatos, elCont, idInd, vista) {
    console.log(losDatos);
     $.each(losDatos, function(idx,val){
//         console.log(val);
       $.each(val.datos, function(index,value){
           losDatos[idx].datos[index].respuesta = $('<textarea />').html(losDatos[idx].datos[index].respuesta).text();
       });       
    });
    console.log(losDatos);
...
function bolasSimples(miDiv, miDato, idInd, laVista, escala, carga) {
    ...
    //Radio del pie
    var arc = d3.svg.arc()
        .outerRadius(radiusPie - mBol)
        .innerRadius(0);

    var labelArc = d3.svg.arc()
        .outerRadius(radiusPie - 5)
        .innerRadius(radiusPie - mBol);
